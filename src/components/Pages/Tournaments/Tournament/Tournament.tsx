...
function Tournament({ item, index }: TournamentProps): ReactElement {
  ...
  const tournamentTypeLabel =
    {
      NUMBERED: t("tournament.types.numbered"),
      FREEDOM: t("tournament.types.freedom"),
      "FIGHT NIGHT": t("tournament.types.fightNight"),
      PAST: t("tournament.types.past"),
    }[item.type] ?? item.type;

  const weightKey = UfcWeightClass[item.weight];
  const weightClassLabel = t(`weights.${weightKey}`);
      ...
      >
        ...
        <div className={styles.info}>
          <div>
            ...
            <p className={styles.weight}>{weightClassLabel}</p>
          ...
          <div className={styles.rightGroup}>
            <p
              className={`${styles.type} ${
                item.type === "NUMBERED" || item.type === "FREEDOM"
                  ? styles.typeNumbered
                  : item.type === "FIGHT NIGHT"
                  ? styles.typeFN
                  : ""
              }`}
            >
      ...
      <div className={`${styles.fights} ${isOpen ? styles.fightsOpen : ""}`}>
        <div className={styles.fightsInner}>
          {item.fights.map((fight: FightEntry, i: number) => {
            const fightWeightClassLabel = t(
              [
                `fighters.categories.${fight.weightClass}`,
                `weights.${fight.weightClass}`,
              ],
            ...
            return (
              <div
                ...
                className={`${styles.fightRow} ${
                  fight.isMain ? styles.fightRowMain : ""
                } ${fight.isTitle ? styles.fightRowTitle : ""}`}
                style={{ animationDelay: `${i * 55}ms` }}
              >
                <div className={styles.fightWeightClass}>
                  {fightWeightClassLabel}
                </div>
                ...
                <div className={styles.fightMatchup}>
                  <span
                    className={`${styles.fighterName} ${
                      isPast && fight.result
                        ? fight.fighter1.includes(fight.result)
                          ? styles.winner
                          : styles.loser
                        : ""
                    }`}
                  ...
                  <span className={styles.fightVs}>{t("tournament.vs")}</span>
                  <span
                    className={`${styles.fighterName} ${
                      isPast && fight.result
                        ? fight.fighter2.includes(fight.result)
                          ? styles.winner
                          : styles.loser
                        : ""
                    }`}
                ...
                {!isPast && (
                  <div className={styles.fightResultAnnounced}>
                    <span>{t("tournament.announced")}</span>
                  </div>
                )}
              </div>
            );
